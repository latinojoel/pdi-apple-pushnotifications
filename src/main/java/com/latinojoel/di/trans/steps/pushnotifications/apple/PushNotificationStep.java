...
public class PushNotificationStep extends BaseStep implements StepInterface {
        ...
        if (first) {
            ...
            for (int i = 0; i < meta.getCustomFieldsDataPush().size(); i++) {
                ...
                if (data.customFieldsDataPushValueNrs.get(i) < 0) {
                    throw new KettleStepException(BaseMessages.getString(PKG,
                            "ApplePushNotification.Exception.FieldRequired", meta.getCustomFieldsDataPush().get(i)));
                }
            }
            for (int i = 0; i < meta.getLocalizedArgumentsDataPush().size(); i++) {
                ...
                if (data.locArgDataPushValueNrs.get(i) < 0) {
                    throw new KettleStepException(BaseMessages.getString(PKG,
                            "ApplePushNotification.Exception.FieldRequired", meta.getLocalizedArgumentsDataPush()
                                    .get(i)));
                }
            }
    ...
    private Date sendPush(RowMetaInterface rowMeta, Object[] r) throws KettleFileException, KettleValueException {
        ...
        ApnsServiceBuilder apnsServiceBuilder = APNS.newService().withCert(fileInputStream, data.certificatePassword)
    ...
    private void cachePosition() throws KettleStepException {
        if (meta.getDeviceTokenField() != null && data.indexOfDeviceTokenField < 0) {
            ...
            if (data.indexOfDeviceTokenField < 0) {
                final String message = "Unable to find table name field [" + realDeviceToken + "] in input row";
                logError(message);
        ...
        if (meta.getAlertBodyField() != null && data.indexOfAlertBodyField < 0) {
            ...
            if (data.indexOfAlertBodyField < 0) {
                final String message = "Unable to find table name field [" + realAlertBody + "] in input row";
                logError(message);
                throw new KettleStepException(message);
            }
        }
        ...
        if (meta.getActionLocalizedKeyField() != null && data.indexOfActionLocalizedKeyField < 0) {
            final String realActionLocalizedKey = environmentSubstitute(meta.getActionLocalizedKeyField());
            ...
            if (data.indexOfActionLocalizedKeyField < 0) {
                final String message = "Unable to find table name field [" + realActionLocalizedKey + "] in input row";
                logError(message);
                throw new KettleStepException(message);
            }
        }
        ...
        if (meta.getLocalizedKeyField() != null && data.indexOfLocalizedKeyField < 0) {
            ...
            if (data.indexOfLocalizedKeyField < 0) {
                final String message = "Unable to find table name field [" + realLocalizedKey + "] in input row";
                logError(message);
                throw new KettleStepException(message);
            }
        }
        ...
        if (meta.getLaunchImageField() != null && data.indexOfLaunchImageField < 0) {
            ...
            if (data.indexOfLaunchImageField < 0) {
                final String message = "Unable to find table name field [" + realLaunchImage + "] in input row";
                logError(message);
                throw new KettleStepException(message);
            }
        }
