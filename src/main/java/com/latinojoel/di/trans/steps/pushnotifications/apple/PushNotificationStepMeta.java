...
public class PushNotificationStepMeta extends BaseStepMeta implements StepMetaInterface {
    ...
    localizedKeyField, launchImageField, certificatePathField, certificatePasswordField, responseField,
    ...
    public String getXML() {
        ...
        for (int i = 0; i < customFieldsDataPush.size(); i++) {
            ...
            retval.append("          ").append(XMLHandler.addTagValue("customFieldsStream", customFieldsStream.get(i)));
        ...
        for (String localizedArgument : localizedArgumentsDataPush) {
            retval.append("        <field>").append(Const.CR);
            retval.append("          ").append(XMLHandler.addTagValue("localizedArgumentsDataPush", localizedArgument));
            retval.append("        </field>").append(Const.CR);
        }
        retval.append("    </localizedArguments>").append(Const.CR);
    ...
    public void readRep(Repository rep, ObjectId idStep, List<DatabaseMeta> databases, Map<String, Counter> counters)
        ...
        try {
            ...
            for (int i = 0; i < nrRows; i++) {
                customFieldsStream.add(Const.NVL(rep.getStepAttributeString(idStep, i, "customFieldsStream"), ""));
                customFieldsDataPush.add(Const.NVL(rep.getStepAttributeString(idStep, i, "customFieldsDataPush"), ""));
            }
    ...
    public void saveRep(Repository rep, ObjectId idTransformation, ObjectId idStep) throws KettleException {
        try {
            ...
            rep.saveStepAttribute(idTransformation, idStep, "actionLocalizedKey", actionLocalizedKeyField);
            ...
            rep.saveStepAttribute(idTransformation, idStep, "certificatePassword", certificatePasswordField);
            ...
            final int nrRows = customFieldsStream.size() < customFieldsDataPush.size() ? customFieldsDataPush.size()
    ...
    public void readData(Node stepnode) throws KettleXMLException {
        try {
            ...
            final int nrlocalizedArgumentsRows = XMLHandler.countNodes(localizedArgumentsFields, "field");
            localizedArgumentsDataPush = new ArrayList<String>();
            for (int i = 0; i < nrlocalizedArgumentsRows; i++) {
                final Node knode = XMLHandler.getSubNodeByNr(localizedArgumentsFields, "field", i);
                localizedArgumentsDataPush.add(XMLHandler.getTagValue(knode, "localizedArgumentsDataPush"));
            }
        } catch (Exception e) {
    ...
    public void check(List<CheckResultInterface> remarks, TransMeta transmeta, StepMeta stepMeta,
        ...
        if (prev == null || prev.size() == 0) {
            ...
            if (certificatePathField == null || "".equals(certificatePathField) || "0".equals(certificatePathField)) {
                cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, BaseMessages.getString(PKG,
            ...
            if (errorFound) {
                ...
                if (localizedArgumentsDataPush.size() > 0) {
                    ...
                    cr = new CheckResult(CheckResultInterface.TYPE_RESULT_WARNING, BaseMessages.getString(PKG,
                            "ApplePushNotification.CheckResult.LocalizedArguments.NoFieldsEntered"), stepMeta);
                    remarks.add(cr);
                }
            }
